...
(function($, window) {
	...
	window.addEventListener('tap', function(e) {
		...
		if (action && action.classList.contains('mui-action-back1')) {
			$.back();
...
(function($, window) {
	...
	$.addBack({
		...
		handle: function() {
			...
			if (parent) {
				parent.evalJS('mui&&mui.back1();');
			} else {
				wobj.canBack(function(e) {
					//by chb 暂时注释，在碰到类似popover之类的锚点的时候，需多次点击才能返回；
					if (e.canBack) { //webview history back1
						window.history.back();
...
(function($, name) {
	...
	var handle = function(event, target) {
		...
		if (className && ~className.indexOf(CLASS_ACTION)) {
			if (target.classList.contains('mui-action-back1')) {
				event.preventDefault();
			}
			return target;
		}
